...
class Songer_Admin {
	...
	public function enqueue_styles() {
		 ...
		 * defined in Songer_Loader as all of the hooks are defined
		 * in that particular class.
		 *
		 * The Songer_Loader will then create the relationship
		 * between the defined hooks and the functions defined in this
		 * class.
		 */

        if ( 'settings_page_songer' == get_current_screen() -> id ) {
            // CSS stylesheet for Color Picker
            wp_enqueue_style( 'wp-color-picker' );
            // CSS stylesheet for thickbox
            wp_enqueue_style('thickbox');
            wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/songer-admin.css', array('wp-color-picker', 'thickbox'), $this->version, 'all' );
        }
	}
	...
	public function enqueue_scripts() {
        ...
        if ( 'settings_page_songer' == get_current_screen() -> id ) {
            wp_enqueue_media();
            wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/songer-admin.js', array( 'jquery', 'wp-color-picker' ), $this->version, false );         
        }		
...
public function display_plugin_setup_page() {
    include_once( 'partials/songer-admin-display.php' );
}
...
public function validate($input) {
     ...
     * @since    1.0.0
     */
     private function songer_login_logo_css(){
         if(isset($this->songer_options['login_logo_id']) && !empty($this->songer_options['login_logo_id'])){
             $login_logo = wp_get_attachment_image_src($this->songer_options['login_logo_id'], 'thumbnail');
             $login_logo_url = $login_logo[0];
             $login_logo_css  = "body.login h1 a {background-image: url(".$login_logo_url."); width:200px; height:150px; background-size: contain;}";
             return $login_logo_css;
         }
     }
     
     // Get Background color is set and different from #fff return it's css
     private function songer_login_background_color(){
         if(isset($this->songer_options['login_background_color']) && !empty($this->songer_options['login_background_color']) ){
             $background_color_css  = "body.login{ background:".$this->songer_options['login_background_color']."!important;}";
             return $background_color_css;
         }
     }
     // Get Button and links color is set and different from #00A0D2 return it's css
     private function songer_login_button_color(){
         if(isset($this->songer_options['login_button_primary_color']) && !empty($this->songer_options['login_button_primary_color']) ){
             $button_color = $this->songer_options['login_button_primary_color'];
     ...
     public function songer_login_css(){
         if( !empty($this->songer_options['login_logo_id']) || $this->songer_login_background_color() != null || $this->songer_login_button_color() != null){
             echo '<style>';
             if( !empty($this->songer_options['login_logo_id'])){
                   echo $this->songer_login_logo_css();
             }
             if($this->songer_login_background_color() != null){
                   echo $this->songer_login_background_color();
             }
             if($this->songer_login_button_color() != null){
                   echo $this->songer_login_button_color();
             }
             echo '</style>';
         }
     }
